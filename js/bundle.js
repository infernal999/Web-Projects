...
function slider({container, slide, nextArrow, prevArrow, totalCounter, currentCounter, wrapper, field}) {
    ...
    function deleteSymbolsCSS(str) {
        return +str.replace(/\D/g, '');
    }

    next.addEventListener('click', () => {
        if (offset == deleteSymbolsCSS(width) * (slides.length - 1)) {
            offset = 0;
        } else {
            offset += deleteSymbolsCSS(width); 
        }
    ...
    prev.addEventListener('click', () => {
        if (offset == 0) {
            offset = deleteSymbolsCSS(width) * (slides.length - 1);
        } else {
            offset -= deleteSymbolsCSS(width);
        }
    ...
    dots.forEach(dot => {
        dot.addEventListener('click', (e) => {
            ...
            slideIndex = slideTo;
            offset = deleteSymbolsCSS(width) * (slideTo - 1);
